...
let parse (input: string[]) = 
    ...
    |> Array.map (function 
        | Success (((lights, buttons), joltage), _, _) -> lights |> charToInt, buttons |> List.map toInt, joltage
        | _ -> failwith "nope")

let shortest_path op (lights, buttons: int list, _) =
    ...
    let rec do_walk (queue: PriorityQueue<int * int, int>) =
        ...
        //printfn " cnt: %d, status: %A, presses: %A" presses lights_status buttons_pressed
        ...
        buttons
        |> List.iter (fun p -> 
            // printfn " adding %A, %A" (op lights_status p) (p::buttons_pressed)
